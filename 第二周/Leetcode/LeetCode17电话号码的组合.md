...
[17电话号码的组合](https://leetcode-cn.com/problems/letter-combinations-of-a-phone-number/)
...
class Solution {
    ...
    void search(string s, string digits, int i, map<char, string> map)  //s是每一次生成的字符串，digits是输入的数字，i是深度（level）
    {
        //终止条件
        if (i == digits.size()) {
            res.push_back(s);
